...
function Set-UIPreference {
    ...
    Write-Output 'You should relog for changes to take effect.'
}
...
function Set-BitLocker {
    ...
    if ($PSCmdlet.ShouldContinue('Continue?', 'Encrypting C: drive.')) {
    ...
    if ($drives) {
        foreach ($drive in $drives) {
            if ($PSCmdlet.ShouldContinue('Continue?', "Automatically unlock $drive at boot?")) {
